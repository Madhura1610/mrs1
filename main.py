...
def rec_pref(genre1 ,genre2,lang,myear):
    ...
    temp_year = { df["title"][i]: df["release_date"][i] for i in range(len(df["title"]))}
    i=0
    rec_movies=[]
    rec_title=[]
    rec_lang=[]
    ...
    if request.method == 'POST':
        ...
        if login_user:
            ...
            if check_password_hash(login_user['password'], _password):
                ...
                genre1=str(res1)
                genre2=str(res2)
                rec_titles=rec_pref(genre1,genre2,lang,year)
